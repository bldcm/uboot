...
/*****************************************************************************/
#if defined(CONFIG_405GP) || defined(CONFIG_405CR) || defined(CONFIG_440) || defined(CONFIG_405EP) || defined(CONFIG_405L)
...
void early_serial_init(void)
{
	 ...
	 * values that are subject to the 1/2 opb clock constraint
	 */
	serial_divs (CONFIG_BAUDRATE, &udiv, &bdiv);
	

	reg |= udiv << CR0_UDIV_POS;	/* set the UART divisor */
#endif

// haerti	mtsdr (UART0_SDR,0x20000018);  // Div=14

	//out8 (UART0_SDR + UART_LCR, 0x80);
	mtsdr (UART0_SDR,reg);


	out8 (ACTING_UART0_BASE + UART_LCR, 0x80);	/* set DLAB bit */
// haerti	out8 (ACTING_UART0_BASE + UART_DLL,   3);	/* set baudrate divisor */
// haerti	out8 (ACTING_UART0_BASE + UART_DLM, 0x0);/* set baudrate divisor */
	out8 (ACTING_UART0_BASE + UART_DLL, bdiv);	/* set baudrate divisor */
	out8 (ACTING_UART0_BASE + UART_DLM, bdiv >> 8);/* set baudrate divisor */
	...
	out8 (ACTING_UART0_BASE + UART_FCR, 0x00);	/* disable FIFO */
	out8 (ACTING_UART0_BASE + UART_MCR, 0x00);	/* no modem control DTR RTS */
	in8 (ACTING_UART0_BASE + UART_LSR);	/* clear line status */
	in8 (ACTING_UART0_BASE + UART_RBR);	/* read receive buffer */
	out8 (ACTING_UART0_BASE + UART_SCR, 0x00);		


// Toggle with RTS/DTR
	for (i=0; i<10; i++) {
		out8 (ACTING_UART0_BASE + UART_MCR, 0x03);	// set RTS and DTR 
		for (j=0; j<10000; j++) 
			;
		out8 (ACTING_UART0_BASE + UART_MCR, 0x00);	// set RTS and DTR 
		for (j=0; j<10000; j++) 
			;
	}
	
	// Send Break
	out8 (ACTING_UART0_BASE + UART_LCR, 0x43);	// enable break 
	for (j=0; j<100000; j++) 
		;
	out8 (ACTING_UART0_BASE + UART_LCR, 0x03);	// disable break 
/* set scratchpad */
	out8 (ACTING_UART0_BASE + UART_IER, 0x00);	/* set interrupt enable reg */
	...
	// Send a characters
//	for (j=0; j<10; j++) {
	out8 (ACTING_UART0_BASE + UART_THR, 'A'); // Send first character 
//	}
...
#if defined(CONFIG_405L)
  unsigned short br_reg;

  /* Disable DMA, set CPU::UART clock divisor to 18 */
  mtdcr (CPC0_UCR, 0x00001212);

  /* If UART clocking turns out to be too inaccurate, use
     table from pp 20-3 in 405L manual, with variable
     CPU::UART dividers */

  br_reg = (((((gd->cpu_clk/16)/18) * 10) / gd->baudrate) + 5) / 10 ;
...
udiv=0;
#ifdef CFG_EXT_SERIAL_CLOCK
  bdiv = ((CFG_EXT_SERIAL_CLOCK/16) / gd->baudrate);
#else
  bdiv = (((((clk/16)/18) * 10) / gd->baudrate) + 5) / 10 ;
#endif

  if (gd->baudrate == 230400) bdiv = 3;  
  mtdcr (CPC0_UCR, 0x00001212); 

  out8(UART0_BASE + UART_LCR, 0x80);  /* set DLAB bit */
  out8(UART0_BASE + UART_DLL, (bdiv & 0x00ff));  /* set divisor for 9600 baud */
  out8(UART0_BASE + UART_DLM, ((bdiv & 0xff00) >> 8));  /* set divisor for 9600 baud */
  out8(UART0_BASE + UART_LCR, 0x03);  /* line control 8 bits no parity */
#else /* !CONFIG_405L */
