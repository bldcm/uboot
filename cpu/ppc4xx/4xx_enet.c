 ...
 */
#if (CONFIG_COMMANDS & CFG_CMD_NET) && (!defined(CONFIG_405) || defined(CONFIG_405L)) && !defined(CONFIG_IOP480)
...
//printf("ef_ptr->source_addr:%x dev->enetaddr:%x hw_p->txbuf_ptr:%x len:%i\n",ef_ptr->source_addr, dev->enetaddr,hw_p->txbuf_ptr,len);
...
//printf("%s/%d  hw_p->hw_addr:%x\n",__FUNCTION__,__LINE__,hw_p->hw_addr);
	...
	msr = mfmsr();
//	printf("msr=%08x \n",msr);
	mtmsr(msr & ~0x00028000); // disable Interrupts 

//printf("%s/%d  temp_txm0:%x\n msr=%x\n",__FUNCTION__,__LINE__,temp_txm0, mfmsr());

	__asm__ volatile ("eieio");


	//*((volatile unsigned int *)0xef600e08) = 0x80000000;
	 out32 (EMAC_TXM0 + hw_p->hw_addr, temp_txm0 | EMAC_TXM0_GNP0);
