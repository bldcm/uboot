...
int
do_empty (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	// dummy fuction for comment beginning with '#'
	return 0;
}

static cmd_tbl_t  __empty_cmd = {"empty",0,0,do_empty,NULL};
...
cmd_tbl_t *find_cmd (const char *cmd)
{
	...
	if ((strlen (cmd)>0) && (cmd[0]=='#')) 
		return &__empty_cmd; // null action only dummy for script comment
	/*
