...
#ifdef CONFIG_TCLS1
extern int call_appl (ulong addr,int argc, char *argv[]);
#endif
...
# error Unknown CPU type
	...
	case IH_TYPE_STANDALONE:
		...
		if (argc > 2) {
			// jump load address
			call_appl ((ulong )appl,argc-1,&argv[1]);
			}
		else {
			call_appl ((ulong )appl,argc,argv);
			}
#else
		(*appl)(argc-1, &argv[1]);
#endif
	...
	*kbd = *(gd->bd);
	...
	if (argc >= 3) {
		...
		if (hdr->ih_magic  != IH_MAGIC) {
			puts ("Bad Magic Number\n");
#ifdef CONFIG_TCLS1
			lcd_print (0,  "* Image error * ");
			lcd_print (16, "Run diagnostics ");
#endif
