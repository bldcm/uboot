...
check_bootcmd(void)
{
	...
	var=getenv (BOOTCMD);
	if (var){
		while(address=strstr(var,TC16FLASHADDRESS)) {
			address++; //skip space
			for (i=0;i<8 && isxdigit(*(address+i));i++);
			if ((i==8)&& (*(address+i)==' ' || *(address+i)=='\0'))
				return -1; // find flass addres for TC16
			var=address+i;
		}
...
void check_env_timestamp (void)
{
	char *str_version=getenv (NAME_ENV_TIMESTAMP);
	if (str_version) {
		switch(str_version[5]) {
		case 'N': 
		{
			// New environment set status to Test
			str_version[5]='T';
			setenv (NAME_ENV_TIMESTAMP,str_version);
			saveenv();
			break;
		}
		...
		case 'T':
		{
			...
			setenv (NAME_ENV_TIMESTAMP,str_version);
			saveenv();
			printf("Environment with invalid timestamp:%s -> system reset!\n",str_version);
...
extern int ext_saveenv(int pos,uchar ldelete);
int do_saveenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	int pos=0;
	int ret=0;
	char *par[4] = {  "setenv",NAME_ENV_TIMESTAMP, NULL,NULL };
	DECLARE_GLOBAL_DATA_PTR;

// CHECK flash address in bootcmd var
	if (check_bootcmd()<0) {
		puts ("Wrong flash address in bootcmd: ");
		bootcmd_var=getenv (BOOTCMD);
		puts (bootcmd_var);
		putc ('\n');
		udelay(30000000);
		return 1;
	...
	if (argc<2) 
	{
		...
		_do_setenv (0, 3, par);
		ldelete=0;
		pos=gd->env_flash_pos+1;
		if (pos>=CFG_ENV_NUMBER)
			pos=0;
		gd->env_flash_pos=pos;
	}

#endif
	ret= (ext_saveenv(pos,ldelete) ? 1 : 0); 
	if (ret==0) 
		printf ("Saving Environment to %s successfully.\n", env_name_spec);
...
U_BOOT_CMD(
    ...
    ((CONFIG_COMMANDS & (CFG_CMD_ENV|CFG_CMD_FLASH)) == \
      (CFG_CMD_ENV|CFG_CMD_FLASH))
U_BOOT_CMD(
	saveenv, CFG_MAXARGS, 1,	do_saveenv,
