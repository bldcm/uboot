...
/*  ----------------------------------------------------------------------  */
/*  1.0   29.Jun.01  JH      jbi_load_jbc_file() for TC16                   */
/*  1.1    8.Jul.01  JH      Auch fuer DOS, Windows; zum Debuggen           */
/*  1.2   13.Jul.01  JH      jtag_io() INT, OUT Makros fuer JTAG mit PC LPT */
/*  1.3   14.Nov.05  CH      JTAG- Programmieren fuer Slot0 und Slot1       */
/****************************************************************************/
...
extern int reset_flash_and_phy (void);
...
/* We've uniform RAM access, so we don't need these .. */
#define CONSTANT_AREA
#define XDATA_AREA       /* external RAM accessed by 16-bit pointer */
#define PDATA_AREA       /* external RAM accessed by 8-bit pointer */
#define IDATA_AREA       /* internal RAM accessed by 8-bit pointer */
#define RDATA_AREA       /* internal RAM registers with direct access */
#define BIT char         /* internal RAM single bit data type */
...
#define JBI_MAX_PROGRAM_SIZE (31 * 1024)        /* when JBC file is in RAM */
...
/*
*  JAM Language Mnemonics
*/
const char *mnem[] = {
  /*00*/  "NOP","DUP","SWP","ADD","SUB","MULT","DIV","MOD",
  /*08*/  "SHL","SHR","NOT","AND","OR","XOR","INV" ,"GT",
  /*10*/  "LT","RET","CMPS","PINT","PRNT","DSS","DSSC","ISS",
  /*18*/  "ISSC","VSS","VSSC","VMPF","DPR","DPRL","DPO","DPOL",
  /*20*/  "IPR","IPRL","IPO","IPOL","PCHR","EXIT","EQU","POPT",
  /*28*/  "","","","","","","","",
  /*30*/  "","","","","","","","",
  /*38*/  "","","","","","","","",
  /*40*/  "PSHL","PSHV","JMP","CALL","NEXT","PSTR","VMAP","SINT",
  /*48*/  "ST","ISTP","DSTP","SWPN","DUPN","POPV","POPE","POPA",
  /*50*/  "JMPZ","DS","IS","DPRA","DPOA","IPRA","IPOA","EXPT",
  /*58*/  "PSHE","PSHA","DYNA","","","","","",
  /*60*/  "","","","","","","","",
  /*68*/  "","","","","","","","",
  /*70*/  "","","","","","","","",
  /*78*/  "","","","","","","","",
  /*80*/  "COPY","REVA","DSC","ISC","WAIT","VS","","",
  /*88*/  "","","","","","","","",
  /*90*/  "","","","","","","","",
  /*98*/  "","","","","","","","",
  /*A0*/  "","","","","","","","",
...
*/
unsigned long RDATA_AREA jbi_stack_ptr;
...
#else
clock_t jbi_timer_count;
clock_t jbi_timer_seconds;
#endif
...
/****************************************************************************/
#ifdef __MSDOS__
extern int GPIO0_BASE;
#define  GPIO0_OR   0
#define  GPIO0_IR   1
#define  IN(a)     inportb((unsigned)a)
#define  OUT(a,d)  outportb((unsigned)a, (unsigned char)d)
#else
#define  IN(a)     (*a)
#define  OUT(a,d)  (*a = d)
#endif
...
#if (defined __MSDOS__) || (defined WIN32)
extern FILE *file;
#define jbi_comm_getchar getch
#else
#define jbi_comm_getchar serial_getc
#define putchar putc
#endif
...
#elif (defined WIN32)
#include <winbase.h>
#define jbi_delay Sleep
#else
#define jbi_delay udelay
#endif /* PROC_8051 */
...
#if __MSDOS__
#define TMS_BIT 0x02         /* Pin  #3, D1 */
#define TDI_BIT 0x40         /* Pin  #8, D6 */
#define TDO_BIT 0x80         /* Pin #11, Busy */
#define TCK_BIT 0x01         /* Pin  #2, D0 */
#elif 1   /* big endian */
#define TMS_BIT 0x00008000   /* Pin #16 */
#define TDI_BIT 0x00000100   /* Pin #23 */
#define TDO_BIT 0x00000080   /* Pin #24 */
#define TCK_BIT 0x00010000   /* Pin #15 */
#else  /* little endian */
#define TMS_BIT 0x00800000   /* Pin #16 */
#define TDI_BIT 0x00010000   /* Pin #23 */
#define TDO_BIT 0x80000000   /* Pin #24 */
#define TCK_BIT 0x00000100   /* Pin #15 */
#endif

BIT jbi_jtag_io(BIT tms, BIT tdi, BIT read_tdo)
{
  volatile unsigned long *gpio_out=(unsigned long *) (GPIO0_BASE + GPIO0_OR);
  volatile unsigned long *gpio_in=(unsigned long *) (GPIO0_BASE + GPIO0_IR);
  BIT tdo = 0;

  if (tms) OUT(gpio_out, IN(gpio_out) | TMS_BIT);  /* set TMS signal */
  else OUT(gpio_out, IN(gpio_out) & ~TMS_BIT);

  if (tdi) OUT(gpio_out, IN(gpio_out) | TDI_BIT);  /* set TDI signal */
  else OUT(gpio_out, IN(gpio_out) & ~TDI_BIT);

  if (read_tdo)
  {
    tdo = (IN(gpio_in) & TDO_BIT) ? 1 : 0;
  }

  /* strobe TCK signal */
  OUT(gpio_out, IN(gpio_out) | TCK_BIT);
  jbi_delay (1);
  OUT(gpio_out, IN(gpio_out) & ~TCK_BIT);
  jbi_delay (1);
...
void jbi_export(unsigned char FILE_AREA *key, long value)
{
  printf("Export: key = \"%s\", value = %ld\n", key, value);
}

void XDATA_AREA *jbi_malloc(unsigned int size)
{
  char XDATA_AREA *ptr;
...
unsigned short jbi_get_program_word(unsigned int x)
{
  unsigned char FILE_AREA *ptr;
  unsigned short ret;

  ptr = jbi_program + x;

  ret = ((unsigned short)ptr[0] << 8) |
...
void jbi_start_timer(void)
{
  ...
  EA = 1;
  TR0 = 1;
#elif __MSDOS__ || WIN32
  jbi_timer_count = time(NULL);
#else
  jbi_timer_count = get_timer (0);
#endif
}
...
void jbi_stop_timer(void)
{
  ...
  TR0 = 0;
#elif __MSDOS__ || WIN32
  jbi_timer_seconds = time(NULL) - jbi_timer_count;
#else
  jbi_timer_seconds = (get_timer (0)-jbi_timer_count)/100;
#endif
}
...
unsigned short jbi_read_packed(unsigned char bits)
...
/****************************************************************************/
{
  unsigned short XDATA_AREA result = 0;
  unsigned char shift = 0;

  while (result != 0xffff && bits > 0)
  {
...
/****************************************************************************/
{
  ...
  unsigned int XDATA_AREA tmp_in_index=0;
  unsigned int XDATA_AREA tmp_out_index=0;
  unsigned char XDATA_AREA tmp_bits_avail=0;
...
/****************************************************************************/
{
  ...
  if (variable_id != jbi_current_variable_id)
  {
    ...
    value = (unsigned int)GET_DWORD(offset + 3);
    jbi_current_variable_id = variable_id;
    jbi_current_page = -1;
    jbi_bits_avail = CHAR_BITS;
    jbi_in_length = (unsigned int)GET_DWORD(offset + 7);
...
(
  ...
  unsigned short XDATA_AREA *error_address,
...
/****************************************************************************/
{
  ...
  unsigned char FILE_OR_XDATA_AREA * XDATA_AREA unknown_ptr=NULL;
  unsigned char ch;
  ...
  */
  ...
  printf ("\nJAM file header information:\n");
  printf ("First Word:    %8.8lX\n", first_word);
  printf ("String Table:  %8.8lX\n", string_table);
  printf ("Symbol Table:  %8.8lX\n", symbol_table);
  printf ("Data Section:  %8.8lX\n", data_section);
  printf ("Code Section:  %8.8lX\n", code_section);
  printf ("Debug Section: %8.8lX\n", debug_section);
  printf ("Required RAM:  %8.8lX\n", ram_required);
  printf ("Symbol Count:  %8.8lX\n\n", symbol_count);
#endif
    ...
    else
    {
      ...
      if (PrintSymbols) {
        for (i=0; filename[i]; i++)
          SymFileName[i] = filename[i];
        SymFileName[i++] = '.';
        SymFileName[i++] = 'S';
        SymFileName[i++] = 'Y';
        SymFileName[i++] = 'M';
        SymFileName[i] = 0;
        file = fopen(SymFileName, "wt");
        if (file == NULL) {
          printf ("Cannot open symbol file!\n");
          PrintSymbols = 0;
        }
        fprintf (file, "Symbols:\n--------\n\n");
        printf ("Writing symbol file ... ");
      }
#else
      if (PrintSymbols)
        printf ("Symbols:\n--------\n\n");
      ...
      for (i = 0; i < (unsigned int) symbol_count; ++i)
      {
        ...
        name = (char*)&jbi_program[string_table + name_id];

        /* print symbol information */
#if __MSDOS__ || WIN32
        if (PrintSymbols) {
          fprintf (file, "Name:    \"%s\"\n", name);
          fprintf (file, "Offset:  0x%4.4X\n", offset);
          fprintf (file, "Name ID: %i\n", name_id);
          fprintf (file, "Flags:   0x%2.2X: ", attributes[i]);
          fprintf (file, "%s/",(attributes[i]&0x20)?"temp":"decl");
          fprintf (file, "%s/",(attributes[i]&0x10)?"int":"bool");
          fprintf (file, "%s/",(attributes[i]&0x08)?"array":"scalar");
          fprintf (file, "%sinit/",(attributes[i]&0x04)?"":"not ");
          fprintf (file, "%scompr/",(attributes[i]&0x02)?"":"not ");
          fprintf (file, "%s\n",(attributes[i]&0x01)?"rw":"ro");
          fprintf (file, "Value:   0x%lX\n", value);
          fprintf (file, "Size:    %i\n\n", variable_size[i]);
        }
#else
        if (PrintSymbols) {
          ...
          printf ("Name ID: %i\n", name_id);
          printf ("Flags:   0x%2.2X: ", attributes[i]);
          printf ("%s/",(attributes[i]&0x20)?"temp":"decl");
          printf ("%s/",(attributes[i]&0x10)?"int":"bool");
          printf ("%s/",(attributes[i]&0x08)?"array":"scalar");
          printf ("%sinit/",(attributes[i]&0x04)?"":"not ");
          printf ("%scompr/",(attributes[i]&0x02)?"":"not ");
          printf ("%s\n",(attributes[i]&0x01)?"rw":"ro");
          printf ("Value:   0x%lX\n", value);
          printf ("Size:    %li\n\n", variable_size[i]);
        }
...
//  printf ("\n");
    ...
    case 0x45: /* PSTR */
      ...
      */
      ...
      jbi_strncpy(&message_buffer[count],
        (char *)&jbi_program[string_table + args[0]],
    ...
    if (status != JBIC_SUCCESS)
    {
      ...
      *error_address = pc = (unsigned int) (opcode_address - code_section);
      printf ("offset in file: %4.4X, addr: %4.4X, opcode: %2.2X, args: ",
        opcode_address, pc, opcode);
      pc ++;
      if (arg_count == 0)
        printf ("no args\n\n");
      else {
...
(
  unsigned short XDATA_AREA *offset,
  char XDATA_AREA *key,
...
/****************************************************************************/
{
  ...
  unsigned short XDATA_AREA note_strings = 0L;
  unsigned short XDATA_AREA note_table = 0L;
  unsigned short XDATA_AREA note_count = 0L;
  ...
  *  Read header information
  */
  note_strings  = (unsigned short) GET_DWORD(8);
  note_table    = (unsigned short) GET_DWORD(12);
  note_count    = (unsigned short) GET_DWORD(44);

  if (note_count > 0)
  {
    note_ptr = (char *)&jbi_program[note_strings];
...
(
  unsigned short XDATA_AREA *expected_crc,
  unsigned short XDATA_AREA *actual_crc
)
...
/****************************************************************************/
{
  unsigned short XDATA_AREA local_expected, local_actual;
  unsigned short shift_reg = 0xffff;
  unsigned char  j, feedback, databyte;
  unsigned short XDATA_AREA crc_section;
  unsigned int i;

  crc_section = (unsigned short) GET_DWORD(32);
...
JBI_RETURN_TYPE jbi_load_jbc_file(void)
{
  ...
  unsigned short XDATA_AREA expected_crc;
  unsigned short XDATA_AREA actual_crc;
  unsigned short XDATA_AREA note_offset = 0;
  unsigned short XDATA_AREA address = 0;
  unsigned short XDATA_AREA count;
  signed char ch, value, pos, size, type, offset;
  ...
  if (!error)
  {
    ...
    if (count == (jbi_get_program_dword(32) + 2L))
    {
...
{
  unsigned short XDATA_AREA note_offset = 0;
  unsigned short XDATA_AREA error_address = 0;
  JBI_RETURN_TYPE XDATA_AREA crc_result = JBIC_SUCCESS;
  JBI_RETURN_TYPE XDATA_AREA exec_result = JBIC_SUCCESS;
  unsigned short XDATA_AREA expected_crc = 0;
  unsigned short XDATA_AREA actual_crc = 0;
  ...
  char CONSTANT_AREA banner[] =
    "Jam Byte Code Player Version 1.0 - JH V"
    VER
    "\nCopyright (C) 1998 Altera Corporation\n";

#ifdef JBC_FILE_IN_RAM
  char CONSTANT_AREA usage_message[] =
    "Commands: Load, CRC, Notes, ID, Erase, Blchk, Pgm, Verify, All, Quit ";
#else
  /* load command is not available when JBC file is in ROM area */
  char CONSTANT_AREA usage_message[] =
    "Commands: CRC, Notes, ID, Erase, Blankcheck, Program, Verify, All, Quit";
#endif /* JBC_FILE_IN_RAM */
  ...
  */
  ...
  {
    printf("\n> ");
    buffer[0] = jbi_toupper(jbi_comm_getchar());
    putchar (buffer[0]);
    putchar ('\n');
...
//	  if ((exit_code == 0) && ((buffer[0] == 'P') || (buffer[0] == 'p'))) {
	/* Doesn't hurt anyway .. */
		printf ("Resetting PHY and flash ... ");
//		reset_flash_and_phy (); /* reset flash and phy after programming */
		printf ("done!\n");
//	  }
        }
        else
        ...
        */
        hours = jbi_timer_seconds / (60 * 60);
        minutes = (jbi_timer_seconds / 60) % 60;
        seconds = jbi_timer_seconds % 60;
...
#if !__MSDOS__ && !WIN32
int do_jtag (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
char jtagrunmode;
  ...
  if (argc > 1)
    {
      jtagrunmode = argv[1][0];
      switch(jtagrunmode)
      {
        case '0': *((unsigned int*)0xEF600700) = 0x000C0000;
	          *((char*)0XF01007FC) = 0;
	          break;
	case '1': *((unsigned int*)0xEF600700) = 0x000C0000;
	          *((char*)0xF01007FC) = 0x80;
                  break;
      }
    }
    else
      *((unsigned int*)0xEF600700) = 0x000C0200;



  jbi_main ();
  reset_flash_and_phy (); /* reset flash and phy after jbi_main() */
  return 0;
}

#if 0 // not supported (CONFIG_COMMANDS & CFG_CMD_BSP)

U_BOOT_CMD(
	jtag,	2,	0, do_jtag,
	"jtag    - start jam bytecode interpreter\n","");
#endif
