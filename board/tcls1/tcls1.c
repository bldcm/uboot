...
extern int unprotect_flashes (void);

int board_early_init_f (void)
{
   /*-------------------------------------------------------------------------+
   | Interrupt controller setup for the Walnut board.
   | Note: IRQ 0-15  405GP internally ggd_t *gd;enerated; active high; level sensitive
...
int checkboard (void)
{
	...
	printf ("TOPCALL Line Server One");

	puts ("Board: ");

	if (!s || strncmp (s, "TCLS1", 9)) {
...
{
	return  CFG_SDRAM_SIZE; //spd_sdram (0);
}
...
{
/* Data sheet for AMD flash says 
      reset pulse width minimum: 500 ns.
      reset high before read: 50 ns (not 100% tested). 

   For PHY: didn't find information. */

	volatile char *epld = (volatile char *) 0xf01007fc;
	*epld = 0x0;  /* reset */
	udelay (5000);
	*epld = 0xc;  /* un-reset */
	udelay (5000);

	unprotect_flashes ();   /* For Intel flashes */
	return 0;
}
