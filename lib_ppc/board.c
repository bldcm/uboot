...
#ifdef CONFIG_TC16
#include "../board/tc16/lcd.h"
#endif

#ifdef CONFIG_TC18
#include "../board/tc18/lcd.h"
#endif
...
static int init_func_i2c (void)
{
	...
	puts ("ready\n");
#ifdef CONFIG_TC18
//	workaround for winbond enable fan 
	puts ("WINBOND :   ");
	i2c_reg_write(WINBOND, 0x4d, 0x25);  /* Make Pin 4 an output pin */
	i2c_reg_write(WINBOND, 0x5c, 0x19);  /* Enable PWM for pin 4 */ 
	i2c_reg_write(WINBOND, 0x5a, 0xff);  /* Select full speed */
	puts ("ready\n");
#endif
	return (0);
}
...
#ifdef CONFIG_TC16
    extern int enable_fans (void);
    extern int reset_flash_and_phy (void);
#endif
#ifdef CONFIG_TC18
    extern int  dsp_jtag_init(void);
    extern void check_env_timestamp (void);
#endif
#if defined(CONFIG_TCLS1)
int lcd_print_version(void)
{
	lcd_print (0,  "TCBIOS  V" VERSION);
	lcd_print (16,  "                ");
	return 0;
}
	#define SHOW_LCD_UBOOT_PROGRESS(nr)  lcd_print (16,  "   boot step: "); lcd_print (30, #nr) ;

#else
	#define SHOW_LCD_UBOOT_PROGRESS(nr)
#endif
...
SHOW_LCD_UBOOT_PROGRESS(1) ;
	...
	debug ("ResetVector from EPROM=%x\n", *(int *)0xfffffffc);
...
SHOW_LCD_UBOOT_PROGRESS(2) ; // befor relocate
...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	gd->flags |= GD_FLG_RELOC;	/* tell others: relocation done */
        ...
        SHOW_LCD_UBOOT_PROGRESS(3) ;  // after relocate
...
SHOW_LCD_UBOOT_PROGRESS(4) ;   // after tarp_init
...
SHOW_LCD_UBOOT_PROGRESS(5) ;  // after flash init 
...
# endif
	...
	#ifdef CONFIG_TC18
	/* check timestamp of environment */
	check_env_timestamp ();
	#endif
...
#ifdef CONFIG_TC16
// only temporary while ppcboot has no structure bi_enet2addr and 
// the linux kernel used bi_pci_enetaddr instead of bi_enet2addr
s = getenv ("ethaddr2");

for (i = 0; i < 6; ++i) {
	bd->bi_pci_enetaddr[i] = s ? simple_strtoul (s, &e, 16) : 0;
	if (s)
		s = (*e) ? e + 1 : e;
}
...
SHOW_LCD_UBOOT_PROGRESS(6) ;   // befor pci_init
...
SHOW_LCD_UBOOT_PROGRESS(7) ;   // after pci_init
...
SHOW_LCD_UBOOT_PROGRESS(8) ;    // befor main loop
